...
public:
    void inputTransaksiBanyak() {
        ...
        double totalArr[MAX_DATA];
        double beratArr[MAX_DATA];
        ...
        if (jumlah > MAX_DATA) {
            jumlah = MAX_DATA;
            cout << "Jumlah transaksi melebihi kapasitas, dibatasi menjadi "
                 << MAX_DATA << " transaksi.\n";
        }
        ...
        for (int i = 1; i <= jumlah; i++) {
            ...
            totalArr[i - 1] = laundry.getTotalBayar();
            beratArr[i - 1] = laundry.getBeratKg();
        }

        HitungTotalRekursif HTR;
        double grandTotal = HTR.hitungTotal(totalArr, jumlah);

        HitungBeratRekursif HBR;
        double totalBerat = HBR.hitungBerat(beratArr, jumlah);
